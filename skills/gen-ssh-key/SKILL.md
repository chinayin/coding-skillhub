...
Output directory precedence: `--out-dir` > `SSH_KEY_OUTPUT_DIR` in a sibling `.env` > current directory.
When neither `--out-dir` nor `SSH_KEY_OUTPUT_DIR` is set, keys land in the current directory and a notice is printed to stderr.
For first use, `cp .env.example .env` and point it at a central directory (e.g. `~/.ssh/team-keys`).
...
./gen-ssh-key.sh jumpserver --comment "you@example.com"
./gen-ssh-key.sh jumpserver --passphrase-file ./pp.txt   # encrypt the private key
./gen-ssh-key.sh jumpserver --out-dir ~/.ssh/team-keys
./gen-ssh-key.sh jumpserver --force               # overwrite same-name keys
./gen-ssh-key.sh jumpserver --json                # machine-readable output (pure JSON on stdout)
./gen-ssh-key.sh jumpserver --dry-run             # show the plan only
./gen-ssh-key.sh jumpserver -v                    # verbose diagnostics on stderr
./gen-ssh-key.sh --tool ssh-keygen jumpserver     # force ssh-keygen (skip puttygen)
./gen-ssh-key.sh --version                        # version
./gen-ssh-key.sh -h                               # help
```
